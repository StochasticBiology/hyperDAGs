...
void random_walkers(arma::mat& rw, int n_walkers, arma::vec A_val, arma::vec A_row_ptr, arma::vec A_col_idx, int n_traits){
  ...
  for(int i=0; i<n_walkers; i++){
    ...
    while(t < n_traits){
      ...
      //      printf("-- %i %i > %i %i\n", t, n_traits, state, start_idx+choice);
      //	printf("%i %i %i\n", state, partners[start_idx+choice], row_col_to_idx(state, partners[start_idx+choice], A_row_ptr, A_col_idx));
      //	printf("%f\n", A_val(row_col_to_idx(state, partners[start_idx+choice], A_row_ptr, A_col_idx)));
...
/*
  A function for making a constrained transiton matrix given a hypercube with a given dimension.
  Input variables:
  - arma::vec A_val: The vector for storing the transition matrix values
  - arma::vec A_row_ptr: The row pointer vector for the transition matrix
  - arma::vec A_col_idx: The column index vector for the transition matrix
  - int n_traits: The number of traits in the system
  - int *edgelist: a list of constrained edges (src1, dest1, src2, dest2, ...)
  - int nedgelist: the number of constrained edges
  Output:
  - The updated version of A with uniform transiton from each node
*/
void constrained_transition_matrix(arma::vec& A_val, arma::vec& A_row_ptr, arma::vec& A_col_idx, int n_traits, int *edgelist, int nedgelist){
  vector<int> n_partners;
  vector<int> c_partners;
  vector<int> partners;
  possible_transitions(n_partners, c_partners, partners, n_traits);
  ...
  for(int i=0; i<mypow2(n_traits); i++){
    ...
    c = 0;
    nfriends = 0;
    // populate a list of "friends": destination nodes in our edge list for which the current node is the source
    for(eli = 0; eli < nedgelist; eli++)
      {
	if(edgelist[2*eli+0] == i)
	  {
	    friends[nfriends++] = edgelist[2*eli+1];
	  }
      }
    for(int j=0; j<n_end_vertices; j++){
      int j2 = partners[k];
      if(nfriends == 0 || nfriends == n_end_vertices)
	{
	  // no, or all,  "friends" in our edge list => uniform outgoing edges
	  thisval = 1./n_end_vertices;
	}
      else
	{
	  // some "friends" in our edge list => nonzero weights only for the friends
	  thisval = 0.01/(n_end_vertices-nfriends);
	  for(eli = 0; eli < nfriends; eli++)
	    {
	      if(friends[eli] == j2)
		thisval = 0.99/nfriends;
	    }
	}
      A_val(r+c) = thisval;
      //      printf("i %i j2 %i r %i c %i val %f\n", i, j2, r, c, A_val(r+c));
...
*/
#ifndef _USE_CODE_FOR_R
void run_inference(vector<string>& data, int L, int n_boot, string name, double& time, int rw_boot, int constrained, int *edgelist, int nedgelist, double eps){
  #else
  List run_inference(vector<string>& data, int L, int n_boot, string name, double& time, int rw_boot, int constrained, int *edgelist, int nedgelist, double eps){
  #endif
  ...
  if(constrained) {
    constrained_transition_matrix(A_val, A_row_ptr, A_col_idx, L, edgelist, nedgelist);
  } else {
  ...
  adapted_baum_welch(A_val, A_row_ptr, A_col_idx, data_bw, data_count, 1000, itr, eps, L, false, false);
  ...
  for(int i=0; i<n_boot; i++){
   ...
   if(constrained) {
    ...
    adapted_baum_welch(A_val, A_row_ptr, A_col_idx, new_data, data_count, 1000, itr, eps, L, false, false);
...
#ifndef _USE_CODE_FOR_R
 void run_inference_longitudinal(vector<string>& data, vector<int>& data_count, int L, int n_boot, string name, double& time, int rw_boot, int constrained, int *edgelist, int nedgelist, double eps){
#else
   List run_inference_longitudinal(vector<string>& data, vector<int>& data_count, int L, int n_boot, string name, double& time, int rw_boot, int constrained, int *edgelist, int nedgelist, double eps){
#endif
  ...
  arma::mat rw(L,L, arma::fill::zeros);
        random_walkers(rw, 10000, A_val, A_row_ptr, A_col_idx, L);
     cout << "Initial ordering matrix from sampling:\n";
  cout << rw << endl;
     
  auto t1 = std::chrono::high_resolution_clock::now();
  adapted_baum_welch(A_val, A_row_ptr, A_col_idx, data, data_count, 1000,itr, eps, L, false, false);
  auto t2 = std::chrono::high_resolution_clock::now();
  double duration_seconds = std::chrono::duration<double>(t2 - t1).count(); //Measure time
  time += duration_seconds;
...
double sum_vector(vector<double> vec){
 ...
 void read_edges(char *fname, int *edgelist, int *n)
 {
   FILE *fp;
   int tmp1, tmp2;
...
int main(int argc, char** argv){
  ...
  vector<string> data;
  vector<int> data_count;
  int *edgelist, nedgelist, constrained;
  double eps;
  
  edgelist = (int*)malloc(sizeof(int)*100000);
  ...
  if(oldform == 1)
      ...
      for(i = 1; i < argc; i+=2)
	{
	  if(strcmp(argv[i], "--obs\0") == 0) strcpy(obsfile, argv[i+1]);
 	  ...
 	  else if(strcmp(argv[i], "--edgelist\0") == 0) { constrained = 1; strcpy(edgestr, argv[i+1]); }
  	  else if(strcmp(argv[i], "--eps\0") == 0) { eps = atof(argv[i+1]); }
  ...
  if(constrained)
    {
      read_edges(edgestr, edgelist, &nedgelist);
    }
  ...
  if(cross_sectional == 1){
    ...
    run_inference(data, L, n_boot, labelstr, time, rw_boot, constrained, edgelist, nedgelist, eps);
  }
  else if(cross_sectional == 0){
    import_data_longitudinal(obsfile, data, data_count, &L);
    cout << "From " << obsfile << " I read " << data.size() << " entries and am assuming " << L << " features\n\n";
    run_inference_longitudinal(data, data_count, L, n_boot, labelstr, time, rw_boot, constrained, edgelist, nedgelist, eps);
  }
