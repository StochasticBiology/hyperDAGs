...
for(dynamics in dyn.set) {
  ancnames[[dynamics]] = c()
  descnames[[dynamics]] = c()

  if(dynamics == "max.spread") {
    descnames[[dynamics]] = rep(binary_strings_with_k_ones(L, floor(L/2)), length.out = tree.size-1)
    ancnames[[dynamics]] = rep(binary_strings_with_k_ones(L, 0), length.out = tree.size-1)
  } else {
    ...
    while(length(to.do) > 0) {
      ...
      for(i in to.do) {
        ...
        for(j in this.outgoing.edges) {
          ...
          ancnames[[dynamics]] = c(ancnames[[dynamics]], anc.str)
          descnames[[dynamics]] = c(descnames[[dynamics]], desc.str)
  ...
  #ancnames = unlist(lapply(x[1:128], paste0, collapse=""))
  #solns[[dynamics]] = simplest_arborescence(ancnames)
}
